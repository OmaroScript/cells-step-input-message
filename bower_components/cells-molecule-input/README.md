...
### Native keyboard visibility
Also, setting the `inputMode` attribute to numeric will show numeric keyboard for mobile devices.
It can have the following values: none, text, decimal, numeric, tel, search, email, url, verbatim
```html
<cells-molecule-input label="Your name" inputMode="numeric"></cells-molecule-input>
```
...
| --cells-molecule-input-info-message-wrapper-icon-color      | Info icon button color                                             | #5bbeff
| --cells-molecule-input-info-message-iron-icon-color         | Color for icon in error message                                    | #f35e61
| --cells-molecule-input-text-info-color                      | Info message text color                                            | #121212
### @apply
